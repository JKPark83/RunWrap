...
enum HomeBriefingEngine {
    ...
    static func briefing(runs: [RunSummary],
                         growth: GrowthState,
                         report: WeeklyReport?,
        ...
        if let line = overloadLine(report: report) { return line }

        // ② 컨디션 변화 (배터리 → 심박 효율 순)
        if let line = conditionLine(battery: battery, report: report) { return line }

        // ③ 꾸준함 — 공백(시무룩)이 먼저, 그다음 칭찬
        if let line = consistencyLine(runs: runs, growth: growth,
                                       report: report, weeklyGoal: weeklyGoal, now: now) {
            return line
        }
    ...
    private static func overloadLine(report: WeeklyReport?) -> String? {
        guard let report else { return nil }

        if let acwr = report.acwr, acwr.tone == .overload {
            let ratio = String(format: "%.1f", acwr.ratio)
            return "평소 감당하던 양의 \(ratio)배를 달렸어요. 이번 주는 한 번 쉬어 가는 게 이깁니다."
        }
        if let distance = report.distance, distance.tone == .overload {
            let pct = String(format: "%+.0f%%", distance.changePct)
    ...
    private static func conditionLine(battery: BatteryReport?, report: WeeklyReport?) -> String? {
        if let battery, battery.tone == .overload || battery.tone == .caution {
            return "체력 배터리 \(battery.level)% — \(battery.headline)"
        }
        if let efficiency = report?.efficiency, efficiency.tone == .improving,
           efficiency.changePct >= 2 {
            let pct = String(format: "%+.1f%%", efficiency.changePct)
            return "같은 심박으로 더 빨리 달리고 있어요 — 심박 효율 \(pct). 몸이 조용히 좋아지는 중입니다."
        }
        if let battery, battery.tone == .improving, battery.level >= 75 {
            return "체력 배터리 \(battery.level)% — \(battery.headline)"
        }
        return nil
    }
    ...
    private static func consistencyLine(runs: [RunSummary], growth: GrowthState,
                                        report: WeeklyReport?, weeklyGoal: Int,
                                        now: Date) -> String? {
        // 시무룩 — 시안 1f(brfS) verbatim 기준. 경과일에 따라 앞머리만 바꾼다
        if growth.isSulky, let days = growth.daysSinceLastRun {
            return "\(gapPhrase(days: days)) 새가 살짝 시무룩하지만, 한 번만 나가면 바로 풀립니다."
        }

        let weekCount = weekRunCount(runs: runs, now: now)
        guard weekCount > 0 else { return nil }  // 이번 주 기록이 없으면 칭찬할 게 없다

        // 시안 1f(brfN) verbatim 기준 문장 — "이번 주 N번째 러닝. 지난주보다 X% 늘었어요 — …"
        // 여기까지 왔다는 건 ①에서 과부하로 걸리지 않았다는 뜻이라, 증가폭은 10% 룰 안이다.
        // 그래서 "딱 좋은 증가폭"이라고 말해도 안전하다.
        if let distance = report?.distance, distance.previous7Km >= 3 {
            let change = distance.changePct
    ...
    private static func gapPhrase(days: Int) -> String {
        switch days {
        case ..<10: "일주일 만이에요."
        case 10..<21: "2주 가까이 쉬었네요."
        case 21..<45: "한 달 가까이 못 뵀어요."
        default: "오랜만이에요."
        }
    }
